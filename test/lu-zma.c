...
long n;
n = (4096 * 16);          /* The size of the matrix */
// long P = DEFAULT_P;          /* Number of processors */
long P;
P = (16 * 16);          /* Number of processors */
// long block_size = DEFAULT_B; /* Block dimension */
long block_size;
block_size = (4096); /* Block dimension */
...
void main()
{
    ...
    mallocinit(p[(0x8000000000),,(0x8000000000) + (0x4000000000)]);
    runner main0();
}
...
//      a[i+j*n] = (double) lrand48()/MAXRAND;
      a[i+j*n] = (double) lrand48()/32767.0;
      if (i == j) {
	a[i+j*n] *= 10;
      }
    }
